...
import { useVendorById, useUpdateVendor } from '@/hooks/useApi';
...
export default function EditVendorPage() {
  ...
  const { data: vendor, isLoading: vendorLoading } = useVendorById(vendorId);
  ...
  useEffect(() => {
    if (vendor) {
      setFormData({
        ...
        contact_person: vendor.data.contact_person || '',
        email: vendor.data.email || '',
        phone: vendor.data.phone || '',
        address: vendor.data.address || '',
        city: vendor.data.city || '',
        state: vendor.data.state || '',
        country: vendor.data.country || 'Bangladesh',
        postal_code: vendor.data.postal_code || '',
        business_type: vendor.data.business_type || 'supplier',
        services_provided: vendor.data.services_provided || '',
        payment_terms: vendor.data.payment_terms || '',
        tax_id: vendor.data.tax_id || '',
        is_active: vendor.data.is_active,
        rating: vendor.data.rating?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateVendor.mutateAsync({ id: vendorId, data: submitData });
  ...
  if (vendorLoading) return <div className="flex justify-center items-center h-64">Loading...</div>;
